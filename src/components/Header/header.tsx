    ...
    <>
            ...
            <ul className="flex space-x-4 mt-4">
              ...
              <li className="mx-2 font-heading">
                ...
                >
                  ...
                  {totalWishlistItems > 0 ? <span className="bg-orange rounded-full text-white px-1 text-xs">{totalWishlistItems}</span> :
                    <span className="bg-orange rounded-full text-white px-1 text-xs">0</span>}Wishlist
                </Link>
              </li>
              <li className="mx-2 font-heading">
                ...
                <Link to="/cart" className="flex items-center text-white hover:text-gray-300">
                  ...
                  {totalItems > 0 ? <span className="bg-orange rounded-full text-white px-1 text-xs">{totalItems}</span> :
                    <span className="bg-orange rounded-full text-white px-1 text-xs">0</span>
                  }Cart
                </Link>
              </li>
            </ul>
      ...
      <nav className="bg-info px-5 h-[71px]">
        <div className="flex justify-between items-center lg:ml-[61px] lg:mr-[49px]">
          <div>
            <select
              className="flex outline-none bg-orange lg:w-[218px] h-[70px] p-2"
              onChange={handleCategoryChange}
            >
              <option value="" className="text-white">Categories</option>
              {categories?.map((category, index) => (
                <option className="text-white" key={index} value={category}>
                  {category}
                </option>
              ))}
            </select>
          ...
          <div className="flex space-x-7">
            <div className="hidden md:flex">
              <Link to="/" className="flex text-black font-medium
               font-heading ">
                Home
            ...
            <div className="hidden md:flex">
              <Link to="/product" className="flex text-black font-heading font-medium">
                product
                <img className="ml-1" src={arrow} alt="" />
              </Link>
            </div>
